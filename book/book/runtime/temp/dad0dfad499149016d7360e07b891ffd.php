<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:78:"D:\phpstudy_pro\WWW\priactise\public/../application/admin\view\user\index.html";i:1650788624;s:66:"D:\phpstudy_pro\WWW\priactise\application\admin\view\uekadmin.html";i:1650782100;}*/ ?>
<!doctype html>
<html lang="en">
    ...
    <div class="panel panel-default">
        ...
        <table class="table-bordered table table-hover">
            ...
            <tr>
                ...
                <td><?php echo (isset($val['nickname']) && ($val['nickname'] !== '')?$val['nickname']:"该用户还未设置昵称"); ?></td>
                <td>
                    <?php if($val['user_image']): ?>
                    <img width="80px" src="/upload/user/{val.user_image}" alt=""> <?php else: ?>
                    <img width="80px" src="/upload/user/head.jpg" alt=""> <?php endif; ?>
                </td>
                <td><?php echo $val['create_time']; ?></td>
                <td><?php echo (isset($val['update_time']) && ($val['update_time'] !== '')?$val['update_time']:"该用户还未更新用户信息"); ?></td>
                <td><?php echo (isset($val['login_time']) && ($val['login_time'] !== '')?$val['login_time']:"该用户还未登录过"); ?></td>
                <td><?php echo (isset($val['autograph']) && ($val['autograph'] !== '')?$val['autograph']:"该用户还未填写个性签名"); ?></td>
                <td><?php echo (isset($val['last_time']) && ($val['last_time'] !== '')?$val['last_time']:"该用户还未开始学习"); ?></td>
                <td><?php echo (isset($val['saids']) && ($val['saids'] !== '')?$val['saids']:"该用户还未填写心里话"); ?></td>
                <td>
                    <?php if($val['status'] == 1): ?>
                    <!-- 点击后状态码变为1 -->
                    <button class="btn btn-success" onclick="status(this,<?php echo $val['id']; ?>,2)">已登录</button> <?php elseif($val['status'] == 3): ?><button class="btn btn-danger" onclick="status(this,<?php echo $val['id']; ?>,2)">黑名单</button> <?php else: ?>
                    <!-- 点击后状态码变为0 -->
                    <button class="btn btn-danger" onclick="status(this,<?php echo $val['id']; ?>,1)">未登录</button> <?php endif; ?>
                </td>
            </tr>
